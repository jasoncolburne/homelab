...
sudo tee /lib/systemd/system/memcached.service << EOF
...
[Service]
NetworkNamespacePath=/run/netns/mem
...
MEM_INFR_IP_ADDRESS=$(rg mem-infr /etc/hosts | cut -d " " -f1)
sudo sed -i "s/^-l .*/-l ${MEM_INFR_IP_ADDRESS}/" /etc/memcached.conf
